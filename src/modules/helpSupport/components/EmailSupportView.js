...
const EmailSupportView = (props) => {
    ...
    const openMail = () => {
        Linking.openURL(`mailto:${dropDownData.support_contact}`)
    };

    useEffect(() => {
        props.fetchDepartment({ 'type': 'emailSupport' });
    }, []);

    return (
        <>
            <Header title={I18n.t('email_support')} />
            <View style={{ margin: convertHeight(15) }}>
                <Text category='h5' style={{ fontWeight: 'bold' }}>{I18n.t('like_to_mail')}</Text>
                <Card style={{ marginTop: convertHeight(7) }} shadow>
                    <TouchableOpacity disabled={dropDownData.support_contact ? false : true} activeOpacity={0.9} onPress={() => openMail()} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 15 }}>
                        <View style={[styles.iconBg, { backgroundColor: theme['color-success-100'] }]}>
                            <Icon name="email" pack="material-community" style={{ height: convertHeight(14), color: theme['color-success-500'] }} />
                        </View>
                ...
                <Card style={{ marginTop: convertHeight(7) }} shadow>
                    <TouchableOpacity activeOpacity={0.9} onPress={() => setDetpartmentFeedBack(!detpartmentFeedBack)} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 15 }}>
                        ...
                        <View style={{ paddingLeft: convertWidth(10) }}>
                            ...
                            <Text style={{ color: theme['text-disabled-color'] }} category='h5' style={{ width: convertWidth(280) }}>{I18n.t('contact_dept_service_offered')}</Text>
                        </View>
                    </TouchableOpacity>
...
export default EmailSupportView;