...
const InvoiceHistoryCollectionView = (props) => {
    ...
    useEffect(() => {
        if (data !== undefined) {
            ...
            const collectionType = _.filter(data[customerNumber], function (o) { return o.summary.collectionTypeId === COLLECTION_TYPE.COLLECTION; });
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            ...
            >
                {
                    COLLECTION_TYPE.COLLECTION === item.summary.collectionTypeId &&
                    <View style={[Platform.OS === 'android' ? styles.androidShadow : styles.iOSShadow, styles.viewStyle, styles.card]}>
                        <TouchableOpacity onPress={() => props.navigateToInvoiceCollectionDetails(item)}>
...
export default InvoiceHistoryCollectionView;
