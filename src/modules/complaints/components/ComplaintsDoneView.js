...
const ComplaintsDoneView = (props) => {

    const { doneComplaints: { refreshing, data, complaintDoneList }, navigation } = props;

    useEffect(() => {
        const focusUnsubscribe = navigation.addListener('focus', () => {
            props.loadDoneComplaints();
        });
    ...
    const renderItem = ({ item, index }) => (
        <Card key={index} shadow style={styles.card}>
            ...
            <View style={{ paddingBottom: convertHeight(5) }}>
                <FlatList
                    horizontal
                    showsHorizontalScrollIndicator={false}
                    data={complaintDoneList}
...
export default ComplaintsDoneView;
