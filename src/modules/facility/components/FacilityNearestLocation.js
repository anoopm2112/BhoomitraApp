...
import FacilityNearestMcfMapView from './FacilityNearestMcfMapView';
...
const FacilityNearestLocation = (props) => {
    const { nearestMcf = [] } = props.facility;
    const { values } = props;
        ...
        Geolocation.getCurrentPosition((coords) => {
            ...
            NetInfo.fetch().then(state => {
                if (state.isInternetReachable) {
                    props.fetchNearestMcf(coords.coords);
                }
            });
        }, (error) => {
    ...
    if (permissionStatus === 'denied') {
        ...
        return (
            <FacilityNearestMcfMapView values={values} nearestMcf={nearestMcf} fetchNearestMcf={props.fetchNearestMcf} onGetLocation={props.onGetLocation} myLocation={userLocation} />
        );
    }
};

export default FacilityNearestLocation;
