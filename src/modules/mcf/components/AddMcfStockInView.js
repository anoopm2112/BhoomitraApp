...
const { Header, SafeAreaView, Input, Content, Card, Text, Button, Picker, Icon, Dropdown, Modal } = components;
const { dimensionUtils: { convertHeight, convertWidth } } = utils;
const { ORGANIZATION_TYPE } = constants;

const AddMcfStockInView = (props) => {
    const { stockInItems = [], mcfAssociations: { associations = [] }, userInfo: { id } } = props;
    const { Item } = Picker;
    ...
    return (
        <SafeAreaView>
            <Header title={I18n.t('mcf_stock_in')} />
            <Formik
                initialValues={{ serviceProvider: '', mcfStockRemarks: '' }}
                validationSchema={validationSchema}
                onSubmit={(values, { resetForm }) => {
                    let data = {
                        remarks: values.mcfStockRemarks,
                        organizationServiceProviderId: values.serviceProvider,
                        transactedBy: id,
                        items: stockInItems
                    };
                    props.addMcfStockIn(data);
                }}
            >
                                ...
                                <Card shadow style={styles.card}>
                                    ...
                                    <View style={{ marginTop: convertHeight(15) }}>
                                        <Input
                                            ...
                                            size='large'
                                            onChangeText={handleChange('mcfStockRemarks')}
                                            value={values.mcfStockRemarks}
                                            multiline={true}
                                            ...
                                            {stockInItems.map((item, index) => {
                                                return (
                                                    <View key={index} style={{ flexDirection: 'row', marginTop: convertWidth(8) }}>
                                                        ...
                                                        <TouchableOpacity onPress={() => props.navigateToAddItemStockIn({ page: 'stockIn', item: item, title: 'mcf_stock_in' })} style={[styles.circle]}>
            ...
            <TouchableOpacity
                onPress={() => { props.navigateToAddItemStockIn({ page: 'stockIn', item: itemData, title: 'mcf_stock_in' }); }}
                activeOpacity={0.8}
...
export default AddMcfStockInView;
