...
const RrfSummaryView = (props) => {
    let { navigation, userRoles } = props;
    const styles = useStyleSheet(themedStyles);
    const theme = useTheme();
    const dashboardItems = [];
    if (hasAccessPermission(userRoles, RESOURCE_MAPPING.MOBILE_HOME, ACTION_MAPPING.MOBILE_HOME.RRF_STOCK_IN_CARD)) {
        dashboardItems.push(
            { id: 1, label: I18n.t('stock_in'), iconName: 'mcf-stock-in', onPress: props.rrfStockIn }
        );
    }
    if (hasAccessPermission(userRoles, RESOURCE_MAPPING.MOBILE_HOME, ACTION_MAPPING.MOBILE_HOME.RRF_SALE_CARD)) {
        dashboardItems.push(
            { id: 2, label: I18n.t('sale'), iconName: 'mcf-sale', onPress: props.rrfSale },
        );
    }
    dashboardItems.push(
        { id: 2, label: I18n.t('shredded_plastic'), iconName: 'mcf-sale', onPress: props.rrfShreddedPlastic },
    );
