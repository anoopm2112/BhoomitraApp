...
const CkcSummary = (props) => {
    let { navigation, userRoles } = props;
    const styles = useStyleSheet(themedStyles);
    const theme = useTheme();
    const dashboardItems = [];
    if (hasAccessPermission(userRoles, RESOURCE_MAPPING.MOBILE_HOME, ACTION_MAPPING.MOBILE_HOME.CKC_PICKUP_CARD)) {
        dashboardItems.push(
            { id: 1, label: I18n.t('pick_up'), iconName: 'mcf-stock-in', onPress: props.ckcPickUp }
        );
    }
    if (hasAccessPermission(userRoles, RESOURCE_MAPPING.MOBILE_HOME, ACTION_MAPPING.MOBILE_HOME.CKC_SALE_CARD)) {
        dashboardItems.push(
            { id: 2, label: I18n.t('sale'), iconName: 'mcf-sale', onPress: props.ckcSale },
        );
    }
            ...
            <SafeAreaView>
                ...
                <View style={styles.flatListContainer}>
                    <View style={styles.viewContainer}>
                        ...
                        <Text appearance='alternative' category='p1'>{I18n.t('pickup_sale')}</Text>
                    </View>
                    <FlatList
                        numColumns={2}
                        horizontal={false}
